(() => {
  ...
  function riskHelp(story) {
    ...
    if (story.category === 'Violencia, Abuso y Acoso') {
      ...
      p.append(violence, ' ofrece información y asesoramiento dentro de su ámbito.');
    }
    box.append(strong, p);
    return box;
  }
  ...
  function openStory(story) {
    ...
    const title = text('h2', story.title);
    title.id = 'story-reader-title';
    const context = text('p', story.context, 'small');
    const disclosure = text('p', library.public_disclosure, 'commercial-disclosure');
    const body = document.createElement('div');
    body.className = 'story-reader-body';
    (story.body || []).forEach(paragraph => body.append(text('p', paragraph)));

    const detail = document.createElement('div');
    detail.className = 'grid two';
    const helped = document.createElement('section');
    helped.className = 'growth-card';
    helped.append(text('h3', 'Qué ayudó en este relato'));
    const helpedList = document.createElement('ul');
    helpedList.className = 'checklist';
    (story.helped || []).forEach(item => helpedList.append(text('li', item)));
    helped.append(helpedList);

    const next = document.createElement('section');
    next.className = 'growth-card';
    next.append(text('h3', 'Siguientes pasos posibles'));
    const nextList = document.createElement('ul');
    nextList.className = 'checklist';
    (story.nextSteps || []).forEach(item => nextList.append(text('li', item)));
  ...
  function card(story) {
    ...
    article.append(text('p', 'Historia de ejemplo', 'label'));
    article.append(text('h2', story.title));
    article.append(text('p', story.excerpt));
    article.append(text('p', story.context, 'small'));
    const button = text('button', 'Leer historia completa', 'button secondary');
    button.type = 'button';
  ...
  function render(category = 'todas') {
    const stories = category === 'todas' ? library.stories : library.stories.filter(story => story.category === category);
    grid.replaceChildren(...stories.map(card));
    status.textContent = `${stories.length} historia${stories.length === 1 ? '' : 's'} de ejemplo.`;
    filters.forEach(button => button.setAttribute('aria-pressed', String(button.dataset.storyFilter === category)));
  }

  filters.forEach(button => button.addEventListener('click', () => {
    if (library) render(button.dataset.storyFilter);
  }));
  dialog.addEventListener('click', event => { if (event.target === dialog) dialog.close(); });

  fetch(DATA_URL, { cache: 'no-store', credentials: 'omit' })
    .then(response => {
      if (!response.ok) throw new Error('story_library_unavailable');
    ...
    .catch(() => {
      status.textContent = 'La biblioteca de relatos no está disponible ahora mismo. Puedes seguir usando Buscar ayuda y Recursos.';
    });
})();
